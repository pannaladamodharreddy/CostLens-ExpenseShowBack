...
def shared_services_spend(**kargs):
    res_dict,cursor = getsharedServiceConfigDetails()
    load_spend_data_if_source_is_API(res_dict,cursor)

def load_spend_data_if_source_is_API(res_dict,cursor):
        if (res_dict['spend_source']):
          usage_src_dict = res_dict['spend_source']
          ...
          for attribute in zip(service_names, usage_src_dict):
            ...
            if usage_dict['type'] == 'API':
                ...
                # API Data Load for
                loadSpendDataOnRedShift(
                    api_endpoint_token, api_search_command, api_endpoint_data, api_path,cursor)

def loadSpendDataOnRedShift(api_endpoint_token, api_search_command, api_endpoint_data, api_path,cursor):

    try:
        data = getsharedServiceConfigDetails.get_data(api_endpoint_token, api_search_command,
                    api_endpoint_data,api_path)

        insert_query_columns = """INSERT INTO dev.public.shared_services_spend(
                metric_date, spend, primary_metric_dimension, secondary_metric_dimension, account_id, cost_center_id, service_name
                    ) VALUES"""
        for row in data:
            noOfRows += 1
            values += """('{}','{}','{}','{}','{}','{}','{}'),""".format(
